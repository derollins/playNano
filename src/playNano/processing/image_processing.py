...
def flatten_afm_frame(frame: np.ndarray, filename: str = "frame", pixel_to_nm: float = 1.0, filter_config: dict = None) -> np.ndarray:
    ...
    try:
        logger.info(f"Flattening frame '{filename}': shape={frame.shape}, dtype={frame.dtype}")
        logger.info(f"Frame stats before flattening: min={np.min(frame)}, max={np.max(frame)}, mean={np.mean(frame)}")

        if frame.size == 0 or np.isnan(frame).any():
            logger.warning(f"Input frame invalid for {filename}: empty or contains NaNs")
            return None
        ...
        if gaussian_filtered is None:
            logger.warning(f"Warning: 'gaussian_filtered' image not found in filters.images for {filename}")
            return None
